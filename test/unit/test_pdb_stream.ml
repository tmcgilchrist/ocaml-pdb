...
let test_pdb_version_roundtrip () =
  let versions =
    [ Pdb.Pdb_stream.VC70; VC80; VC110; VC140; Unknown 12345 ]
  in
  List.iter
    (fun v ->
...
let test_pdb_stream_roundtrip () =
  ...
  Alcotest.(check int) "version" 20140508
    (Pdb.Pdb_stream.pdb_version_to_int info'.version);
  Alcotest.(check int) "signature" 0x5F3A1234
    (Unsigned.UInt32.to_int info'.signature);
  Alcotest.(check int) "age" 1 (Unsigned.UInt32.to_int info'.age);
  (* Verify GUID *)
  Alcotest.(check int) "guid.data1" 0x12345678
    (Unsigned.UInt32.to_int info'.guid.data1);
  Alcotest.(check int) "guid.data2" 0xABCD
    (Unsigned.UInt16.to_int info'.guid.data2);
  Alcotest.(check int) "guid.data3" 0xEF01
    (Unsigned.UInt16.to_int info'.guid.data3);
  Alcotest.(check string) "guid.data4"
    "\x01\x02\x03\x04\x05\x06\x07\x08" info'.guid.data4;
  (* Verify named streams *)
  Alcotest.(check int) "named stream count" 2
    (List.length info'.named_streams);
  Alcotest.(check bool) "has /names" true
    (List.exists
       (fun (n, i) -> n = "/names" && i = 5)
       info'.named_streams);
  Alcotest.(check bool) "has /LinkInfo" true
    (List.exists
       (fun (n, i) -> n = "/LinkInfo" && i = 8)
       info'.named_streams);
  (* Verify features *)
...
let test_pdb_stream_no_features () =
  ...
  let cur = Object.Buffer.cursor obj_buf in
  let info' = Pdb.Pdb_stream.read cur in
  Alcotest.(check int) "version VC70" 20000404
    (Pdb.Pdb_stream.pdb_version_to_int info'.version);
  Alcotest.(check int) "no features" 0 (List.length info'.features);
  Alcotest.(check int) "no named streams" 0
    (List.length info'.named_streams)
...
let test_guid_string () =
  ...
  Alcotest.(check bool) "guid string starts with {" true
    (String.length s > 0 && s.[0] = '{');
  Alcotest.(check bool) "guid string ends with }" true
    (s.[String.length s - 1] = '}')
...
let test_pdb_stream_full_roundtrip_through_msf () =
  ...
  let info' = Pdb.Pdb_stream.read cur in
  Alcotest.(check int) "version through MSF" 20140508
    (Pdb.Pdb_stream.pdb_version_to_int info'.version);
  Alcotest.(check int) "age through MSF" 1
    (Unsigned.UInt32.to_int info'.age);
  Alcotest.(check bool) "named stream /names through MSF" true
    (List.exists
       (fun (n, i) -> n = "/names" && i = 3)
       info'.named_streams)
