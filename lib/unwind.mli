(** Windows structured exception handling / stack unwinding.
    ...
    Windows x86-64 and ARM64 use fundamentally different unwind formats:
    x86-64 uses 2-byte slot-based codes with a fixed header, while ARM64
    uses variable-length byte codes with a bitfield header. Unlike DWARF CFI
...
(** [parse arch cur] parses unwind information from the cursor,
    dispatching to the appropriate architecture-specific parser. *)

val write : Stdlib.Buffer.t -> t -> unit
(** [write buf info] serializes unwind information using the
    appropriate architecture-specific writer. *)
