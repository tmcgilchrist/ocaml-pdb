(** MSF (Multi-Stream File) container reader.
    ...
    PDB files use the MSF format, a page-based virtual filesystem.
    The file is divided into fixed-size blocks (typically 4096 bytes).
    Logical streams are mapped onto sequences of these blocks, which
...
(** {2 Superblock}
    ...
    The superblock occupies the first block of the file and contains
    metadata about the MSF layout. *)
