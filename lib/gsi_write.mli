(** Global/Public Symbol Index (GSI/PSI) writer.
    ...
    Counterpart to {!Gsi}. Builds the hash tables that let consumers look up
    public or global symbols by name. The output is three separate streams
    (symbol record + GSI hash + PSI hash) whose indices feed back into the DBI
    header via {!Dbi_write.write}'s [~sym_record_stream], [~global_stream], and
...
(** [write_gsi buf entries] writes a GSI hash stream (used for both the global
    and public symbol hash tables). *)
...
(** [write_publics_stream buf symbols] writes a complete publics stream
    including the publics header, GSI hash table, and address map. [symbols]
    should be the public symbol records (S_PUB32). *)
...
(** [build_gsi_streams ~publics ~globals] builds the symbol record stream
    (public records followed by global records), the publics hash stream, and
    the globals hash stream. These should be added as MSF streams and their
    indices wired into the DBI header. *)
