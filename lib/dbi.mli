...
(** Section contribution: which bytes of a PE section came from which module.
    Used by the debugger to map addresses back to source files. *)
...
type module_info = {
  ...
  module_sym_stream : int;
      (** MSF stream containing this module's symbol records, or 0xFFFF if
          absent. *)
  sym_byte_size : int;
  ...
  c13_byte_size : int;
      (** Size of the C13 debug-subsection payload following the symbols in
          [module_sym_stream]. *)
  source_file_count : int;
      (** Number of source files in the FileInfo substream. *)
  module_name : string;
      (** Logical module name, usually an object-file path. *)
  obj_file_name : string;
      (** Object file path; may differ from [module_name]. *)
}
(** Per-module metadata describing one compilation unit. *)

type header = {
  version_signature : int32;  (** Always -1 in well-formed PDBs. *)
  version_header : u32;  (** Format version (typically [V70 = 19990903]). *)
  ...
  sym_record_stream : int;
      (** MSF stream containing the concatenated public+global symbol records
          that GSI/PSI index into. *)
  pdb_dll_rbld : int;
  mod_info_size : int;  (** Byte size of the module-info substream. *)
  section_contribution_size : int;
      (** Byte size of the section-contribution substream. *)
  section_map_size : int;  (** Byte size of the section-map substream. *)
  file_info_size : int;
  ...
  flags : int;
  machine : int;  (** PE COFF machine type (e.g. 0x8664 for AMD64). *)
}
...
type optional_debug_header = {
  ...
  new_fpo_data : int;
      (** New-style FPO stream (same wire layout as the C13 FrameData
          subsection). *)
  original_section_header : int;
}
(** Optional debug header: a fixed-length array of stream indices pointing at
    auxiliary streams. [0xFFFF] means "absent." *)
...
(** [parse cur] reads the DBI stream from the cursor position. Raises
    [Object.Buffer.Invalid_format] on truncated input. *)

val module_symbols :
  Msf.t -> module_info -> Codeview_symbols.symbol_record Seq.t
(** [module_symbols msf mod_info] lazily iterates the symbol records of a
